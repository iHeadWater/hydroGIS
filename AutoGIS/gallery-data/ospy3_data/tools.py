...
def __getGeomName(geomType):
    ...
    for const in vars(ogr):
        if (
            const.startswith("wkb")
            and vars(ogr)[const] == geomType
            and not const in ignore
        ):
            return const
    return None
...
def printAtts(layer):
    ...
    if __isSpatial(layer):
        print "The layer has %i features of type %s." % (
            layer.GetFeatureCount(),
            __getGeomName(featureDefn.GetGeomType()),
        )
    else:
    ...
    for i in range(featureDefn.GetFieldCount()):
        formatStr = (
            formatStr
            + "%("
            + featureDefn.GetFieldDefn(i).GetName()
            + ")"
            + str(featureDefn.GetFieldDefn(i).GetWidth())
            + "s     "
        )
    vals = {}
    for i in range(featureDefn.GetFieldCount()):
        vals[featureDefn.GetFieldDefn(i).GetName()] = featureDefn.GetFieldDefn(
            i
        ).GetName()
